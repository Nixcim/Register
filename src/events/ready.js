...
//Status
    ...
    setInterval(() => {
        ...
        client.user.setPresence({ activity: { type: Activity, name: statusMessages.random() }, status: Status });
        console.log(`Botun durumu guncellendi`);
...
//Voice 
    ...
    if (!channel) console.log(`Ses kanalı bulunamadı`);
    else channel.join().then(connection => console.log(`Ses kanalına bağlandı`)).catch(err => console.log(`Ses kanalına bağlanamadı`));
...
//Saving Commands
    ...
    console.log(`Bot komutları kaydetti`);
